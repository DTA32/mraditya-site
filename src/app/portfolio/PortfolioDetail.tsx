...
export default function PortfolioDetail(props: {
  ...
  const statusMapper = [
    ...
    { text: "Scrapped", color: "bg-red-700" },
  ];
  ...
  return (
    <div
      className={`${props.closeFunc ? "fixed inset-0 my-auto mx-auto z-40 w-11/12 lg:w-3/4 h-5/6 lg:h-3/4 rounded-xl" : "flex-1 h-full"} bg-slate-900 flex flex-col lg:flex-row text-white`}
    >
      ...
      <div
        className={`bg-neutral-700 flex place-center
                            ${
                              props.fromModal
                                ? "h-2/6 md:h-1/2 lg:h-full w-full lg:w-[45%] [&:has(>*):not(:has(>*>*))]:lg:w-[75%] rounded-none lg:rounded-l-xl"
                                : "h-[240px] md:h-auto w-full lg:w-1/3"
                            }`}
      >
      ...
      <div className="p-8 gap-10 w-full h-full flex flex-col overflow-y-auto">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-2 w-fit">
            <div
              className={`${statusMapper[props.data.status].color} w-4 h-4 rounded-full`}
            ></div>
